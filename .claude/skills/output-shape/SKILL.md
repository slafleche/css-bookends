...
- **Variants are typed objects, never magic strings.** A book exports a named preset
  ...
  typed options. Pick one as an argument or via a selector; never a bare string.
  ```ts
  color('#3366cc').css(colorFormats.hex);   // one-off variant (argument)
...
- **Output is hardened to its variant** (see `type-hardening`). A selector narrows the
  ...
  one-off `.css(colorFormats.hex)` is too. The configured / escalating default `.css()`
  stays the generic type, since the variant is chosen at runtime.
